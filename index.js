...
const app = express();
app.use(cors({ origin: '*', exposedHeaders: ['X-Etiquetas-Unidas', 'X-Etiquetas-Fallidas', 'X-Etiquetas-Motivo'] }));
...
app.get('/api/despacho/buscar', async (req, res) => {
  try {
    const venta = (req.query.venta || '').trim().replace(/[^0-9A-Za-z_-]/g, '');
...
async function purgarFotoVieja() {
  try {
    ...
    const { error: e1 } = await supabase.from('dep_envios').delete()
      .or('status.in.(delivered,returned,not_delivered),cancelada.eq.true') // terminados: entregados, devoluciones y cancelados
